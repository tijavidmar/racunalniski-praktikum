\begin{frame}{Zaporedja, vrste in limite}
	\begin{enumerate}
		\item 
		Naj bo $\sum_{n=1}^{\infty} a_n $ absolutno konvergentna vrsta in $a_n \ne -1$.
		Dokaži, da je tudi vrsta $\sum_{n=1}^\infty \frac{a_n}{1+a_n}$
		absolutno konvergentna.

		\item
		Izračunaj limito
		$$ \lim_{x \to \infty} \big(\sin(\sqrt{x+1}) - \sin(\sqrt{x})\big)$$

		\item
		Za dani zaporedji preveri, ali sta konvergentni.
		% Pomagajte si s spodnjima delno pripravljenima matematičnima izrazoma:
		% a_n = \sqrt{2+\sqrt{2+\dots+\sqrt{2}}} \qquad
		% b_n = \sin(\sin(\dots(\sin 1)\dots))
		$$
		a_n = \underbrace{\sqrt{2 + \sqrt{2 + \cdots + \sqrt{2}}}}_{n \text{ korenov}}, \qquad 
		b_n = \underbrace{\sin(\sin(\dots(\sin 1)\dots))}_{n \text{ sinusov}}
		$$
	\end{enumerate}
\end{frame}

\begin{frame}{Algebra}
	\begin{enumerate}
		\item
		Vektorja $\vec{c} = \vec{a} + 2 \vec{b} \text{ in } \vec{d} = \vec{a} - \vec{b}$
		sta pravokotna in imata dolžino 1. Določi kot med vektorjema $\vec{a}$ in $\vec{b}$.
		\item 
		Izračunaj
		$ \begin{pmatrix}
			1 & 2 & 3 & 4 & 5 & 6 \\
			4 & 5 & 2 & 6 & 3 & 1
		\end{pmatrix}^{-2000} $
	\end{enumerate}
\end{frame}

\begin{frame}{Velika determinanta}
	Izračunaj naslednjo determinanto $2n \times 2n$, ki ima na neoznačenih mestih ničle.
	%$$
	%\begin{vmatrix}
	%	1 & 1 &        &        &        &        &        &  \\
	%	2 & 2 & \ddots &        &        &        &        &  \\
	%	  & \ddots & \ddots &        &        &        &        &  \\
	%	  &        & \ddots & n-1 & n-1 &        &        &  \\
	%	1 & 2 & \cdots & n-1 & n & n+1 & \cdots & 2n \\
	%	n+1 & n+1 &        &        &        &        &        &  \\
	%	\vdots & \vdots &        &        &        &        &        &  \\
	%	2n & 2n &        &        &        &        &        & 
	%\end{vmatrix}
	%$$
\end{frame}

\begin{frame}{Grupe}
	Naj bo
	$$G = \left\{ z \in \CC \;\middle|\; z = 2^k \bigl(\cos(m \pi \sqrt{2}) + i \sin(m \pi \sqrt{2})\bigr),\; k, m \in \ZZ \right\} $$
	$$H = \left\{(x,y) \in \RR^2 \;\middle|\; x, y \in \ZZ \right\}$$
	\begin{enumerate}
		\item
			Pokaži, da je $G$ podgrupa v grupi $ (\CC \setminus \{0\}, \cdot)$
			neničelnih kompleksnih števil za običajno množenje.
		\item
			Pokaži, da je $H$ podgrupa v aditivni grupi $(\RR^2, +)$
			ravninskih vektorjev za običajno seštevanje po komponentah.
		\item
			Pokaži, da je preslikava $f:H\to G$, podana s pravilom
			$$(x, y) \mapsto 2^x \bigl( \cos(y \pi \sqrt{2}) + i \sin(y \pi \sqrt{2}) \bigr) $$
			izomorfizem grup $G$ in $H$.
	\end{enumerate}
\end{frame}
